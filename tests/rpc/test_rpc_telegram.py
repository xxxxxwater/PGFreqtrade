...
async def test_telegram_pm_status_falls_back_to_read_only_account_view(
    default_conf, update, mocker, table
) -> None:
    """Manual PM positions are shown without becoming strategy Trade rows."""
    default_conf["telegram"]["enabled"] = False
    default_conf["trading_mode"] = "futures"
    default_conf["margin_mode"] = "cross"
    default_conf["stake_currency"] = "USDT"
    default_conf["exchange"]["portfolio_margin"] = True
    default_conf["exchange"]["pair_whitelist"] = ["ETH/USDT:USDT"]
    telegram, _, _ = get_telegram_testobject(mocker, default_conf)
    pm_status = mocker.patch.object(telegram, "_pm_status", new=AsyncMock())
    mocker.patch.object(
        telegram._rpc, "_rpc_trade_status", side_effect=RPCException("no active trade")
    )
    mocker.patch.object(
        telegram._rpc, "_rpc_status_table", side_effect=RPCException("no active trade")
    )
    context = MagicMock()
    context.args = ["table"] if table else []

    await telegram._status(update=update, context=context)

    pm_status.assert_awaited_once_with(update, context)
