...
class Webhook(RPCHandler):
    ...
    def _init_sender_queue(self, queue_maxsize: int = 100) -> None:
        ...
        # Keep a small reserved lane for safety notifications.  Normal entry /
        # exit traffic must never be able to evict WARNING or EXCEPTION alerts.
        self._critical_queue_maxsize = max(10, self._queue_maxsize // 10)
        ...
        self._critical_queue: queue.Queue[dict[str, Any]] = queue.Queue(
            maxsize=self._critical_queue_maxsize
        )
    ...
    def _drain_queue(self) -> None:
        ...
        for message_queue in (self._critical_queue, self._queue):
            while True:
                try:
    ...
    def _enqueue(self, payload: dict[str, Any], *, critical: bool = False) -> None:
        """Queue a payload; reserve capacity and last-resort delivery for alerts."""
        message_queue = self._critical_queue if critical else self._queue
        try:
            message_queue.put_nowait(payload)
        except queue.Full:
            if critical:
                # Preserve emergency visibility even during a notification storm.
                # This is deliberately a rare, last-resort synchronous send: it
                # trades a bounded HTTP timeout for never silently dropping a
                # safety alert.
                try:
                    self._send_msg(payload)
                    self._sent += 1
                    ...
                    self._failed += 1
                    logger.exception("Failed to synchronously send critical webhook message.")
                return
            self._dropped += 1
            logger.warning(
                "Webhook queue full (%d); dropping non-critical message. Total dropped: %d",
                self._queue_maxsize,
                self._dropped,
            )

    def _get_next_payload(self) -> dict[str, Any] | None:
        """Prefer safety notifications without starving regular webhook traffic."""
        try:
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            "module": self.name,
            "queue_size": self._queue.qsize() + self._critical_queue.qsize(),
            "queue_maxsize": self._queue_maxsize,
            "critical_queue_size": self._critical_queue.qsize(),
            "critical_queue_maxsize": self._critical_queue_maxsize,
            "sent": self._sent,
            "failed": self._failed,
    ...
    def send_msg(self, msg: RPCSendMsg) -> None:
        ...
        try:
            ...
            if msg["type"] in {RPCMessageType.WARNING, RPCMessageType.EXCEPTION}:
                self._enqueue(payload, critical=True)
            else:
