...
# Columns added to pm_order_intents by the PM order-consistency redo.
#
...
def migrate_pm_tables(engine: Engine) -> None:
    """
    Add the PM redo columns to a pre-existing pm_order_intents table.

    ``metadata.create_all`` only creates missing tables, never missing columns,
    so databases created by older PM builds need explicit ALTER statements.
    ``PENDING`` was the pre-redo state name.  It is upgraded to ``PREPARED``
    in the same transaction as the column additions.  The model also treats a
    surviving legacy PENDING value as unresolved, so an interrupted/manual
    migration fails closed instead of allowing a duplicate entry.
    """
    ...
    with engine.begin() as connection:
        ...
        if "state" in columns:
            result = connection.execute(
                text(
                    "UPDATE pm_order_intents "
                    "SET state = 'PREPARED' "
                    "WHERE UPPER(state) = 'PENDING'"
                )
            )
