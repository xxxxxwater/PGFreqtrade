...
# MomentumVolumePairList is futures-only (it refuses spot instantiation) and is
# covered by its own dedicated spot + futures tests in test_momentumvolumepairlist.py.
TESTABLE_PAIRLISTS = [
    p for p in AVAILABLE_PAIRLISTS if p not in ["RemotePairList", "MomentumVolumePairList"]
]
...
def test_ticker_cache_honors_shortest_pairlist_refresh_period(mocker, whitelist_conf, markets, tickers):
    """Ticker consumers must not receive a snapshot older than their refresh interval."""
    whitelist_conf["pairlists"][0]["refresh_period"] = 900
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=markets),
        exchange_has=MagicMock(return_value=True),
    ...
    assert freqtrade.pairlists._ticker_cache_ttl == 900
    assert freqtrade.pairlists._ticker_cache.ttl == 900
