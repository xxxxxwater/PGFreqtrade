...
def test_pm_order_intent_crud_and_state_lifecycle():
    ...
    assert loaded.to_dict()["state"] == "PREPARED"

    # ACKED (exchange accepted, evidence stored) also counts as unresolved.
    loaded.state = "ACKED"
    loaded.exchange_order_id = "123"
    loaded.raw_response = '{"orderId": 123}'
    loaded.acked_at = dt_now()
    PMOrderIntent.session.commit()
    assert PMOrderIntent.has_unresolved() is True
    assert PMOrderIntent.get_unresolved()[0].exchange_order_id == "123"

    # LINKED (local Trade/Order committed) no longer blocks new exposure.
    loaded.state = "LINKED"
    loaded.linked_order_id = "123"
    loaded.linked_at = dt_now()
    PMOrderIntent.session.commit()
    assert PMOrderIntent.has_unresolved() is False
    assert PMOrderIntent.has_unresolved_exposure_increasing() is False
    ...
    # Tombstoning (RECONCILED) removes the row.
...
def test_intent_put_failure_blocks_post():
    ...
    exchange._pm_enqueue = MagicMock(
