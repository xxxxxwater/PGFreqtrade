...
class PairListManager(LoggingMixin):
    def __init__(self, exchange, config: Config, dataprovider: DataProvider | None = None) -> None:
        ...
        # Ticker-based pairlists need the live ticker snapshot to be no older
        # than their own refresh period.  The former fixed 30-minute cache made
        # a pairlist configured for e.g. 15 minutes rank stale 24h momentum and
        # quote-volume data for half an hour.  Use the shortest requested
        # interval so a chain of ticker consumers cannot receive stale data.
        ticker_refresh_periods = [
            max(0, int(handler.refresh_period))
            for handler in self._pairlist_handlers
            if handler.needstickers
        ]
        self._ticker_cache_ttl = min(ticker_refresh_periods, default=1800)
        self._ticker_cache: FtTTLCache = FtTTLCache(maxsize=1, ttl=self._ticker_cache_ttl)
    ...
    def _get_cached_tickers(self) -> Tickers:
        tickers = self._ticker_cache.get("tickers")
        ...
        self._ticker_cache["tickers"] = tickers
        logger.info(
            "Pairlist ticker snapshot refreshed: symbols=%s cache_ttl_s=%s fetch_ms=%.1f.",
            len(tickers),
            self._ticker_cache_ttl,
            fetch_ms,
        )
        return tickers
