    ...
    "definitions": {
        "exchange": {
            ...
            "properties": {
                ...
                "portfolio_margin": {
                    "description": (
                        "Enable Binance Portfolio Margin adapter mode. Only supported for "
                        "Binance futures with cross margin and USDT/USDC linear perpetual "
                        "contracts."
                    ),
                    "type": "boolean",
                    "default": False,
                },
                "portfolio_margin_symbol_config_cache_seconds": {
                    "description": (
                        "Seconds to cache the Binance PM tradable-symbol configuration "
                        "(/papi/v1/um/symbolConfig). Default: 300."
                    ),
                    "type": "integer",
                    "minimum": 1,
                    "default": 300,
                },
                "portfolio_margin_stoploss_capability_cache_seconds": {
                    "description": (
                        "Seconds to cache the Binance PM algo-stoploss channel availability "
                        "probe (/papi/v1/um/algo/openAlgoOrders). Default: 300."
                    ),
                    "type": "integer",
                    "minimum": 1,
                    "default": 300,
                ...
                "portfolio_margin_risk": {
                    "description": "Portfolio Margin risk controls.",
                    "type": "object",
                    "additionalProperties": False,
                    "properties": {
                        "min_uni_mmr": {
                            "description": (
                                "Minimum Binance Portfolio Margin uniMMR required before "
                                "opening new non-reduce-only orders."
                            ),
                            "type": "number",
                            "minimum": 0,
                        ...
                        "emergency_stop_uni_mmr": {
                            "description": (
                                "Binance Portfolio Margin uniMMR threshold that force-closes "
                                "all open trades and stops the bot."
                            ),
                            "type": "number",
                            "minimum": 0,
                            "exclusiveMinimum": True,
                        ...
                        "user_stream_enabled": {
                            "description": (
                                "Enable Binance Portfolio Margin user data stream for account "
                                "and order events."
                            ),
                            "type": "boolean",
                            "default": True,
                        ...
                        "user_stream_listen_key_keepalive_minutes": {
                            "description": (
                                "Interval in minutes for proactive Binance Portfolio Margin "
                                "listenKey renewal. Keep below 30 minutes to leave an "
                                "operational buffer before exchange expiry."
                            ),
                            "type": "integer",
                            "minimum": 1,
                            "default": 20,
                        ...
                        "user_stream_health_interval_minutes": {
                            "description": (
                                "Interval in minutes for Binance Portfolio Margin user stream "
                                "health monitoring."
                            ),
                            "type": "integer",
                            "minimum": 1,
                        ...
                        "user_stream_queue_warning_size": {
                            "description": (
                                "Queued Binance Portfolio Margin user stream event count that "
                                "triggers an RPC warning."
                            ),
                            "type": "integer",
                            "minimum": 1,
                            "default": 500,
                        ...
                        "user_stream_disconnected_restart_seconds": {
                            "description": (
                                "Seconds a Binance Portfolio Margin user stream may remain "
                                "disconnected before the bot restarts it."
                            ),
                            "type": "integer",
                            "minimum": 1,
                            "default": 120,
                        ...
                        "user_stream_max_restarts_per_hour": {
                            "description": (
                                "Maximum Binance Portfolio Margin user stream restarts per "
                                "rolling hour before restart attempts are suppressed."
                            ),
                            "type": "integer",
                            "minimum": 1,
                            "default": 3,
                        ...
                        "user_stream_recover_unmatched_orders": {
                            "description": (
                                "Run PM order recovery when a user stream order event does not "
                                "match a local open order."
                            ),
                            "type": "boolean",
                            "default": True,
                        },
                        "monitor_interval_minutes": {
                            "description": (
                                "Interval in minutes for scheduled Binance Portfolio Margin "
                                "risk monitoring."
                            ),
                            "type": "integer",
                            "minimum": 1,
                        ...
                        "heartbeat_risk_cache_seconds": {
                            "description": (
                                ...
                                "risk data used by worker heartbeat logs."
                            ),
                            "type": "integer",
                            "minimum": 1,
                            "default": 300,
                        ...
                        "max_leverage": {
                            "description": (
                                "Maximum leverage allowed for Binance Portfolio Margin "
                        ...
                        "max_total_notional": {
                            "description": (
                                "Maximum total notional value allowed across all open Binance "
                        ...
                        "max_position_notional": {
                            "description": (
                                "Per-pair maximum notional value for Binance Portfolio Margin "
                                "positions. Use '*' as a default cap for dynamic pairlists; "
                                "an exact pair key takes precedence."
                            ),
                            "type": "object",
                            "additionalProperties": {
                                "type": "number",
                        ...
                        "data_failure_exit": {
                            "description": (
                                "When true, an open PM position whose market-data stream "
                                "becomes unhealthy (missing/gapped candles, stale 1h inputs, "
                                "zero volume) is force-closed instead of held. Entries on "
                                "unhealthy data are ALWAYS blocked (unified fail-closed); "
                                "this switch only controls open positions. Default: false "
                                "(hold the position)."
                            ),
                            "type": "boolean",
                            "default": False,
                        ...
                        "snapshot_cache_ttl_s": {
                            "description": (
                                "Seconds to reuse a single-flight account / position snapshot "
                                "before the next PAPI request. Higher values save endpoint "
                                "weight; lower values make risk checks fresher. Default: 3."
                            ),
                            "type": "number",
                            "minimum": 0.1,
                            "exclusiveMinimum": True,
