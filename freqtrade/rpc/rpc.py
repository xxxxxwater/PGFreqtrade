...
class RPC:
    ...
    def _rpc_pm_recover_locked(self) -> dict[str, Any]:
        ...
        return {
            ...
            "intents_linked": intents_report.get("linked", 0),
            "intents_orphaned": intents_report.get("orphaned", 0),
    ...
    def health(self) -> dict[str, Any]:
        ...
        if getattr(self._freqtrade.exchange, "_is_portfolio_margin", lambda: False)():
            ...
            try:
                ...
                pm["signal_ledger_rows"] = PMSignalLedger.session.query(
                    PMSignalLedger
                ).count()
            except Exception:
                logger.debug("Could not include PM outbox / ledger stats in health endpoint.")
            governor = getattr(self._freqtrade.exchange, "_pm_governor_inst", None)
            if governor is not None and hasattr(governor, "stats"):
                pm["papi_governor"] = governor.stats()
            try:
                risk = self._freqtrade.exchange.get_pm_risk_summary()
                if risk.get("enabled"):
